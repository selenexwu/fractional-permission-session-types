...
let rec pp_exp env i exp = match exp with
  ...
  | A.Immut(xs,perm,p) -> "immut " ^ pp_channames xs ^ " { " ^ perm ^ " =>\n" ^ pp_exp_indent env (i+2) p ^ "\n" ^ spaces i ^ "}"
  | A.Continue(xs) -> "continue " ^ pp_channames xs
  | A.Mut(p) -> "mut {\n" ^ pp_exp_indent env (i+2) p ^ "\n" ^ spaces i ^ "}"
  | A.Start(x,perm,p) -> "start " ^ pp_chan x ^ "{" ^ pp_perm perm ^ "} ;\n" ^ pp_exp_indent env i p
...
let pp_exp_prefix exp = match exp with
  ...
  | A.Immut(xs,p,_p) -> "immut " ^ pp_channames xs ^ " { " ^ p ^ " => ... }"
  | A.Continue(xs) -> "continue " ^ pp_channames xs
  | A.Mut(_p) -> "mut { ... }"
  | A.Start(x,perm,_p) -> "start " ^ pp_chan x ^ "{" ^ pp_perm perm ^ "} ; ..."
...
let pp_decl env dcl = match dcl with
  ...
  | A.ExpDecDef(f,(ids,ps,delta,(x,a)),p) ->
    "proc " ^ f ^ "[" ^ pp_list ids ^ "]{" ^ pp_list ps ^ "} : " ^ pp_chantp_list delta ^ " |- "
    ^ pp_label_proto (x,a) ^ " = \n" ^
