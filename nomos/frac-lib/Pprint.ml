...
and pp_proto_simple a = match a with
  ...
  | A.GetPot(pot,a) -> "<" ^ pp_potpos pot ^ "| " ^ pp_proto_simple a
  | A.PayPot(pot,a) -> "|" ^ pp_potpos pot ^ "> " ^ pp_proto_simple a
  | A.Up(a) -> "/\\ " ^ pp_proto_simple a
  | A.Down(a) -> "\\/ " ^ pp_proto_simple a
  | A.FArrow(t,a) -> pp_ftp_simple t ^ " -> " ^ pp_proto_simple a
  | A.FProduct(t,a) -> pp_ftp_simple t ^ " ^ " ^ pp_proto_simple a
  | A.TpName(a) -> a
  | A.FMap(kt,vt) -> "Map<" ^ pp_ftp_simple kt ^ ", " ^ pp_ftp_simple vt ^ ">"
  | A.STMap(kt,vt) -> "Map<" ^ pp_ftp_simple kt ^ ", " ^ pp_proto_simple vt ^ ">"
  | A.Coin -> "coin"
...
let rec pp_exp env i exp = match exp with
  ...
  | A.STMapCreate(mp,kt,vt,p) -> pp_chan mp ^ " <- new Map<" ^ pp_ftp_simple kt ^ ", " ^ pp_proto_simple vt ^ "> ;\n" ^ pp_exp_indent env i p
...
let pp_exp_prefix exp = match exp with
  ...
  | A.STMapCreate(mp,kt,vt,_p) -> pp_chan mp ^ " <- new Map<" ^ pp_ftp_simple kt ^ ", " ^ pp_proto_simple vt ^ "> ; ..."
